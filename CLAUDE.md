...
- `canopy project {dispatch|runners|declare|turns} <project>` — the same one-shot aimed at a REPO rather than an agent (`canopy project dispatch connect-labs --prompt "…"`). Three things differ and each fails silently if you get them wrong, so the command handles them: the turn needs a `--workspace` (its tenant, carried in the `/api/w/<ws>/harness/turns/` path — a project turn has no agent to derive tenancy from); the runner must **declare** the repo in `capabilities.projects` or the turn is accepted and then never claimed by anything (`dispatch` refuses up front rather than enqueueing into a hole; `--declare` / `canopy project declare` fixes the capability in place — both **being retired**, see below) — and it preflights the **tenant it enqueues into** (`GET /api/w/<ws>/harness/runners/`), not the flat route, because canopy-web #509 scoped that read by tenant rather than by who paired what: the flat route now returns the union of every workspace you belong to, so preflighting on it and POSTing to one workspace can pass on workspace A's runners and queue forever in workspace B. Consequently an empty tenant read is a **refusal** ("workspace X has no runners"), not the old `UNKNOWN` warn — `UNKNOWN` survives only for an untenanted read by a caller with no memberships. Rows carry `can_manage` (#509), so a runner you can see but not mutate is reported as "ask its owner" rather than as a `--declare` target that would 404; and it writes **no board task**, because a repo has no agent board. Backed by `src/orchestrator/project_dispatch.py`.
  - **`--declare` / `canopy project declare` are on borrowed time.** canopy-web #513 makes `capabilities["projects"]` **runner-reported** (replaced from the box on every heartbeat — emdash's projects table on a laptop, `RUNNER_PROJECTS` on a cloud runner) and 422s any PATCH carrying `projects`: you make a repo routable by opening it in emdash on that runner, not by declaring it from a client. #513 had **not shipped** as of 2026-07-28 (deployed `HeartbeatIn` carries no `projects`), so the commands still work; when it deploys they fail with that explanation instead of a raw 422, and then get deleted. See issue #433.
