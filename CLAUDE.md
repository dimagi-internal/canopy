...
- `canopy project {dispatch|runners|declare|turns} <project>` — the same one-shot aimed at a REPO rather than an agent (`canopy project dispatch connect-labs --prompt "…"`). Three things differ and each fails silently if you get them wrong, so the command handles them: the turn needs a `--workspace` (its tenant, carried in the `/api/w/<ws>/harness/turns/` path — a project turn has no agent to derive tenancy from); the runner must **declare** the repo in `capabilities.projects` or the turn is accepted and then never claimed by anything (`dispatch` refuses up front rather than enqueueing into a hole; `--declare` / `canopy project declare` fixes the capability in place) — but only when it can SEE a fleet: runner listing is scoped to `paired_by == caller`, so a caller who paired no runner gets `UNKNOWN` (warn + enqueue + verify with `canopy project turns`), never a refusal drawn from an empty list; and it writes **no board task**, because a repo has no agent board. Backed by `src/orchestrator/project_dispatch.py`.
